...
# Database implementation for Web Contractor
...
class LeadRepository:
    ...
    def setup_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_scraping_logs_date ON scraping_logs(scrape_date)"
        )
        cursor.execute(
    ...
    def get_pending_audits(self, limit: int = 10) -> List[Dict]:
        ...
        for row in rows:
            cursor.execute(
                'UPDATE leads SET status = "auditing" WHERE id = ?', (row["id"],)
            )
    ...
    def get_stats(self) -> Dict:
        ...
        # Averages
        avg_pers = cursor.execute(
            "SELECT AVG(personalization_score) FROM email_campaigns WHERE personalization_score IS NOT NULL"
        ).fetchone()[0]
        stats["avg_personalization_score"] = avg_pers if avg_pers else 0.0

        avg_audit = cursor.execute(
            "SELECT AVG(overall_score) FROM audits WHERE overall_score IS NOT NULL"
        ).fetchone()[0]
    ...
    def get_monthly_stats(self, month_prefix: str) -> Dict:
        ...
        # Monthly total
        cursor.execute(
            "SELECT COUNT(*) FROM leads WHERE created_at LIKE ?", (f"{month_prefix}%",)
        )
        monthly_total = cursor.fetchone()[0]

        # By source
        cursor.execute(
            "SELECT source, COUNT(*) FROM leads WHERE created_at LIKE ? GROUP BY source",
            (f"{month_prefix}%",),
        )
        ...
        # By bucket
        cursor.execute(
            "SELECT bucket, COUNT(*) FROM leads WHERE created_at LIKE ? GROUP BY bucket",
            (f"{month_prefix}%",),
        )
    ...
    def get_leads(
        self, page: int = 1, per_page: int = 20, status: str = None, bucket: str = None
    ) -> Dict:
