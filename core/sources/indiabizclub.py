"""Indiabizclub scraper implementation."""
...
class IndiabizclubScraper(BaseScraper):
    """Scraper for Indiabizclub (B2B directory)."""

    SOURCE_NAME = "indiabizclub"
    BASE_URL = "https://www.indibizclub.com"

    SELECTORS = {
        "search_box": "input.search-box",
        "result_card": "div.listing-card",
        "business_name": "h3.listing-title a",
        "phone": "span.contact-phone",
        "email": "span.contact-email",
    ...
    def search(
        ...
        """Search Indiabizclub for businesses matching query.
        ...
        try:
            ...
            self.log(f"Error searching Indiabizclub: {e}", "error")
    ...
    def _extract_email(self, card: Any) -> str | None:
        """Extract email from card."""
        try:
            email_elem = card.query_selector(self.SELECTORS["email"])
            if email_elem:
                return email_elem.inner_text().strip()
        except Exception:
            pass
        return None
...
ScraperRegistry.register(IndiabizclubScraper)
