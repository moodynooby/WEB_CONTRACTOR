"""Audit Pipeline — consolidated agent definitions, pipeline orchestration, and runners.

Single source of truth for:
- Individual audit agents (content, business, technical, performance)
- Pipeline stages (discovery, audit, email gen, refinement, sending)
- AuditResultAggregator
- Session runner

Usage:
    from audit.pipeline import build_full_pipeline, run_pipeline

    pipeline = build_full_pipeline()
    result = run_pipeline(limit=20)
"""
...
from infra.http import fetch_website
...
async def _run_pipeline_session(
    ...
    """Run a pipeline agent session with progress tracking."""
    session_service = InMemorySessionService()
    runner = Runner(agent=agent, app_name=APP_NAME, session_service=session_service)
...
def _build_audit_agent(
    ...
    """Single factory for all audit agents — eliminates duplicated Agent blocks."""
    return Agent(
        name=name,
        model=get_llm_model(),
        instruction=instruction,
        description=description,
        tools=tools or [fetch_website],
...
# ---------------------------------------------------------------------------
# Cached generic audit agents (no lead context)
# ---------------------------------------------------------------------------

@functools.lru_cache(maxsize=1)
def get_content_agent() -> Agent:
    return _build_audit_agent(
...
# ---------------------------------------------------------------------------
# Per-lead audit builder (deduplicated via factory)
# ---------------------------------------------------------------------------

def build_audit_with_lead(lead: dict) -> Agent:
    """Build an Agent that audits a single lead's website."""
    ...
    parallel = Agent(
        name="audit_parallel",
        sub_agents=[
            _build_audit_agent("content_auditor", _content_instruction(bucket, business_name), "Content quality audit.", "content_result"),
            _build_audit_agent("business_auditor", _business_instruction(bucket, business_name), "Business strategy audit.", "business_result"),
            _build_audit_agent("technical_auditor", _technical_instruction(bucket, business_name), "Technical SEO audit.", "technical_result"),
            _build_audit_agent("performance_auditor", _performance_instruction(bucket, business_name), "Performance audit.", "performance_result"),
        ],
...
def build_audit_pipeline() -> Agent:
    """Build the ADK Agent that runs all 4 generic audit agents in parallel."""
    return Agent(
        name="audit_pipeline",
...
async def run_pipeline_async(
    ...
    state = await _run_pipeline_session(agent, message, session_id, progress_callback)
