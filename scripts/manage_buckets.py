...
Features:
- Create new buckets using AI-powered BucketGenerator
...
def create_bucket_non_interactive(business_type: str, locations: str, max_queries: int, max_results: int):
    ...
    print(f"[→] Creating bucket for '{business_type}' in {len(target_locations)} locations...")
...
def delete_bucket():
    ...
    try:
        ...
        confirm = prompt(f"Delete bucket '{bucket_name}' and all related data? (y/N)", "n")
...
def main():
    ...
    parser.add_argument("--create", action="store_true", help="Create bucket non-interactively")
    parser.add_argument("--business-type", type=str, help="Business type (with --create)")
    parser.add_argument("--locations", type=str, help="Comma-separated locations (with --create)")
    parser.add_argument("--max-queries", type=int, default=10, help="Max queries per run (with --create)")
    parser.add_argument("--max-results", type=int, default=50, help="Max results per query (with --create)")
