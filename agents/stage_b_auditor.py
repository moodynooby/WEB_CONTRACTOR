...
class OllamaAuditor:
    ...
    def _test_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.model_name not in model_names and self.model_name != "qwen3:8b":
                    print(
                        f"⚠️  Warning: Model {self.model_name} not found in Ollama tags. Available: {model_names}"
                    )
                return True
            print(
                f"❌ Ollama connection test failed with status: {response.status_code}"
            )
            return False
    ...
    def analyze_website(
        self, business_name: str, metrics: Dict, soup: BeautifulSoup
    ) -> Dict:
...
class TechnicalAuditor:
    ...
    def __init__(self):
        ...
        self.issue_patterns = {
            ...
            "mobile_unfriendly": {
                "pattern": lambda soup: (
                    not soup.find("meta", attrs={"name": "viewport"})
                ),
                "severity": "high",
            ...
            "old_technology": {
                "pattern": lambda soup: bool(
                    soup.find(
                        string=re.compile(
                            r"WordPress\s*[0-4]\.|Joomla\s*1\.|Drupal\s*[6-7]\."
                        )
                    )
                ),
                "severity": "medium",
            ...
            "broken_links": {
                "pattern": lambda links: (
                    len([l for l in links if l.get("status_code", 200) >= 400]) > 0
                ),
                "severity": "medium",
                    ...
                    not any(
                        [
                            soup.find(
                                "a",
                                href=re.compile(
                                    r"facebook|twitter|instagram|linkedin", re.I
                                ),
                            ),
                            soup.find(
                                "div", class_=re.compile(r"testimonial|review", re.I)
                            ),
                        ]
                    )
                ),
            ...
            "accessibility_issues": {
                "pattern": lambda soup: (
                    not any(
                        [soup.find("img", alt=True), soup.find("h1"), soup.find("main")]
                    )
                ),
                "severity": "medium",
    ...
    def audit_website(self, url: str, lead_id: int, business_name: str) -> AuditResult:
        ...
        try:
            ...
            main_response = (
                https_response
                if (https_response and https_response.status_code < 400)
                else http_response
            )

            # If still no valid response or we got a block code, try Selenium
            if not main_response or (
                main_response and main_response.status_code in [403, 401]
            ):
            ...
            llm_analysis = self.llm_auditor.analyze_website(
                business_name, technical_metrics, soup
            )
            ...
            if llm_analysis and "error" not in llm_analysis:
                ...
                print(
                    f"⚠️  Skipping dynamic analysis for {business_name}: {llm_analysis['error']}"
                )
    ...
    def _test_protocols(
        ...
        try:
            ...
            try:
                https_response = self.session.get(
                    https_url, timeout=10, allow_redirects=True, verify=True
                )
                ...
                https_response = self.session.get(
                    https_url, timeout=10, allow_redirects=True, verify=False
                )
                import urllib3
    ...
    def _collect_technical_metrics(
        ...
        metrics = {
            ...
            "has_google_analytics": bool(
                soup.find(src=re.compile(r"google-analytics|gtag", re.I))
            ),
            "has_facebook_pixel": bool(
                soup.find(src=re.compile(r"facebook\.com/tr", re.I))
            ),
            ...
            if href.startswith("http"):
                try:
                    ...
                    link_statuses.append(
                        {"url": href, "status_code": link_response.status_code}
                    )
                except:
            ...
            try:
                ...
                if issue_name == "slow_loading":
                    ...
                    if config["pattern"](link_statuses):
                        broken_links = [
                            l["url"] for l in link_statuses if l["status_code"] >= 400
                        ]
                        issues.append(
                            WebsiteIssue(
                                issue_type=issue_name,
    ...
    def _calculate_overall_score(
        self, issues: List[WebsiteIssue], metrics: Dict
    ) -> float:
    ...
    def _determine_qualification(
        self, overall_score: float, issues: List[WebsiteIssue]
    ) -> bool:
...
class StageBAuditor:
    ...
    def audit_pending_leads(self, batch_size: int = 50) -> Dict:
        ...
        for i, lead in enumerate(pending_leads):
            ...
            try:
                # Perform audit
                audit_result = self.technical_auditor.audit_website(
                    website, lead_id, business_name
                )
    ...
    def _print_audit_summary(self, results: List[AuditResult], qualified_count: int):
        ...
        print(
            f"Qualification Rate: {qualified_count / len(results):.1%}"
            if results
            else "0%"
        )
